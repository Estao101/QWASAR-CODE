...
requests["Find the name of all reviewers who have rated 7 or more stars to their rating order by reviews rev_name (it might have duplicated names :-))"] = "SELECT rev_name FROM reviews r JOIN movies_ratings_reviews mrr ON r.id = mrr.rev_id WHERE mrr.rev_stars >= 7 ORDER BY rev_name;"
...
requests["Find the first and last names of all the actors who were cast in the movies 'Annie Hall', and the roles they played in that production"] = "SELECT a.act_fname, a.act_lname, ma.role FROM actors a JOIN movies_actors ma ON a.id = ma.act_id WHERE ma.mov_id IN (SELECT id FROM movies WHERE mov_title = 'Annie Hall');"
...
JOIN directors d ON d.id = dm.dir_id
...
SELECT a.act_fname, a.act_lname, m.mov_title, CAST(strftime('%Y', m.mov_dt_rel) AS INTEGER) AS release_year
    FROM actors AS a
